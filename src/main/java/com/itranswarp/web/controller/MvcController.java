...
public class MvcController extends AbstractController {
	...
	public ModelAndView index() {
		List<Article> recentArticles = this.articleService.getPublishedArticles(10);//查询最近的10条文章
		List<Topic> recentTopics = this.boardService.getRecentTopicsFromCache();//查询最近的10条话题
	...
	public String locale(@PathVariable("lo") String lo, HttpServletRequest request, HttpServletResponse response) {
		...
		if("zh_CN".equals(lo)){
			this.localeResolver.setLocale(request, response, Locale.CHINA);
		}else if("en".equals(lo)){
			this.localeResolver.setLocale(request, response, Locale.US);
		}
	...
	public ModelAndView signinLocal(@RequestParam("email") String email, @RequestParam("passwd") String password,
		...
		//System.out.println("auth.salt="+auth.salt);
		//System.out.println("auth.passwd="+auth.passwd);
	...
	private void appendGlobalModel(ModelAndView mv) {
		...
		// translator:
		mv.addObject("__translator__", translators.getTranslator(this.localeResolver.resolveLocale(ctx.request)));
